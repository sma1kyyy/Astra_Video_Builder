...
настройки:
- `subtitle_font_size` — размер шрифта, default 40.
- `subtitle_max_chars` — лимит символов в субтитрах, default 250. При превышении текст обрезается с `...`. Полностью весь текст помещается если разбить длинные `tts` на несколько сцен (см. ниже).
- `subtitle_bg_opacity`
- `subplace` — `up | center | down` (default `down`).

> **Лимит TTS.** Поле `tts` в одной сцене не может превышать 200 символов — это валидируется в схеме. Если текст не помещается, разбей озвучку на несколько сцен по смыслу. AI-генератор соблюдает это автоматически.
...
- стартовый экран — три карточки: «Загрузить YAML» / «Конструктор» / «AI-генератор», ниже общая форма для скриншотов;
...
#### AI-генератор (карточка №3)
Опишите сценарий словами — LLM сгенерирует валидный YAML по описанию. Поддерживаются оба режима: **Live Recording** и **Screenshot**.

- **Screenshot**: выберите ранее загруженные скриншоты, опционально добавьте описание для каждого. OCR-распознавание подключается автоматически — LLM получает реальный текст со скриншотов и использует его в `target_text` для аннотаций.
- **Live Recording**: укажите стартовый URL. Под капотом поднимается headless Chrome, LLM получает tool-calling доступ к нему и **реально проходит сценарий** в браузере, прежде чем отдать YAML. Перед возвратом YAML проигрывается в браузере для проверки селекторов (self-verification).

Дополнительные опции в форме:
- голос TTS (`jane`, `zahar`, `off` — без озвучки)
- субтитры (`on` — добавляются автоматически в сцены с tts / `off` — не добавляются)
- язык, разрешение, FPS, браузер (только live)

Кнопка «🚀 Запустить рендер» появляется только для валидного screenshot-YAML (live mode требует физического display server — рендер через web недоступен). Кнопка «Открыть в редакторе» переводит YAML в конструктор для ручной правки. См. также [API_Reference §AI-генератор](API_Reference.md#ai-генератор-yaml-скриптов).
